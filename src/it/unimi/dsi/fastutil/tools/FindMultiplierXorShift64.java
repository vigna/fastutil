...
public class FindMultiplierXorShift64 {
	...
	public final static class Candidate implements Comparable<Candidate> {
		...
		public Candidate( long multiplier, int shift ) {
		...
		public String toString() {
			return "Candidate [multiplier=0x" + Long.toHexString( multiplier ) + ", shift=" + shift + ", bias=" + bias + ", chiSquared=" + chiSquared + "]";
		}
	...
	public static void main( String[] args ) throws InterruptedException {
		...
		Runnable barrierAction = new Runnable() {
			...
			public void run() {
				...
				for( int i = 0; i < Math.min( TOP, array.length ); i++ ) {
					...
					candidates.add( c );
					candidates.add( new Candidate( c.multiplier, c.shift + 1 ) );
					candidates.add( new Candidate( c.multiplier, c.shift - 1 ) );

					candidates.add( new Candidate( c.multiplier << 1 | 1, c.shift - 1 ) );
					candidates.add( new Candidate( c.multiplier << 1 | 1, c.shift ) );
					candidates.add( new Candidate( c.multiplier << 1 | 1, c.shift + 1 ) );

					for( int b0 = 64; b0-- != 1; ) {
						candidates.add( new Candidate( c.multiplier ^ ( 1 << b0 ), c.shift - 1 ) );
						candidates.add( new Candidate( c.multiplier ^ ( 1 << b0 ), c.shift ) );
						candidates.add( new Candidate( c.multiplier ^ ( 1 << b0 ), c.shift + 1 ) );
						for( int b1 = b0; b1-- != 1; ) {
							candidates.add( new Candidate( c.multiplier ^ ( 1 << b0 ) ^ ( 1 << b1 ), c.shift - 1 ) );
							candidates.add( new Candidate( c.multiplier ^ ( 1 << b0 ) ^ ( 1 << b1 ), c.shift ) );
							candidates.add( new Candidate( c.multiplier ^ ( 1 << b0 ) ^ ( 1 << b1 ), c.shift + 1 ) );
						}
					}

					for( long m : new long[] { 0x33333333, 0x55555555, 0x99999999 } ) {
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | m << 32, c.shift - 1 ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | m << 32, c.shift ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | m << 32, c.shift + 1 ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF00000000L | m,  c.shift - 1 ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF00000000L | m, c.shift ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF00000000L | m, c.shift + 1 ) );
					}
					...
					for( int j = 0; j < Math.min( TOP, array.length ); j++ ) {
						...
						candidates.add( new Candidate( c.multiplier ^ d.multiplier | 1, c.shift ) );
						candidates.add( new Candidate( c.multiplier ^ d.multiplier | 1, d.shift ) );
						candidates.add( new Candidate( c.multiplier ^ d.multiplier | 1, c.shift + 1 ) );
						candidates.add( new Candidate( c.multiplier ^ d.multiplier | 1, d.shift + 1 ) );
						candidates.add( new Candidate( c.multiplier ^ d.multiplier | 1, c.shift - 1 ) );
						candidates.add( new Candidate( c.multiplier ^ d.multiplier | 1, d.shift - 1 ) );
						candidates.add( new Candidate( c.multiplier + d.multiplier | 1, c.shift ) );
						candidates.add( new Candidate( c.multiplier + d.multiplier | 1, d.shift ) );
						...
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | d.multiplier & 0xFFFFFFFF00000000L, c.shift ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | d.multiplier & 0xFFFFFFFF00000000L, d.shift ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | d.multiplier & 0xFFFFFFFF00000000L, c.shift + 1 ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | d.multiplier & 0xFFFFFFFF00000000L, d.shift + 1 ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | d.multiplier & 0xFFFFFFFF00000000L, c.shift - 1 ) );
						candidates.add( new Candidate( c.multiplier & 0xFFFFFFFF | d.multiplier & 0xFFFFFFFF00000000L, d.shift - 1 ) );
					}
				}
				...
				for( int r = 8; r-- != 0; ) {
					...
					candidates.add( new Candidate( multiplier, 31 ) );
					candidates.add( new Candidate( multiplier, 32 ) );
					candidates.add( new Candidate( multiplier, 33 ) );
				}
		...
		for ( int i = thread.length; i-- != 0; ) thread[ i ] = new Thread() {
			public void run() {
				...
				for(;;) {
					...
					final int shift = candidate.shift;
					...
					do {
						n++;
						final long mapped = ( multiplier * x ) ^ ( ( multiplier * x ) >>> shift );
						long flippedBitMask = 1;
						for( int f = 64; f-- != 0; ) {
							final long t  = multiplier * ( x ^ flippedBitMask );
							final long flippedMapped = mapped ^ t ^ t >>> shift;
							long bitMask = 1L << 63;
							for( int b = 64; b-- != 0; ) {
		...
		for( int shift = 16; shift <= 48; shift++ ) { 
			candidates.add( new Candidate( 0x3333333333333333L, shift ) );
			candidates.add( new Candidate( 0x5555555555555555L, shift ) );
			candidates.add( new Candidate( 0x9999999999999999L, shift ) );
		}
